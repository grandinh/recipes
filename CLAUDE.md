...
- Web UI: bound to `127.0.0.1:8420` via systemd drop-in (`/etc/systemd/system/recipe-server.service.d/10-hub-bind-loopback.conf`). External access at **`https://ralphsdaily.taile20ebd.ts.net:8445/`** — port-based Tailscale Serve mount (published via `grandin publish recipes 8420 --https-port 8445`). **Do not switch back to a path-based `/recipes` mount:** templates and `app.js` use root-absolute URLs (`/static/…`, `/calendar`, `fetch('/api/…')`), and Tailscale Serve strips the path prefix before proxying — the browser then resolves those URLs against the tailnet root and they hit whatever service owns `/` (currently Glance), not the recipe server. Same failure mode that forced Homepage and Infisical onto port-based mounts. If a pretty path is ever wanted, convert every template/JS absolute URL to `url_for()` with `root_path="/recipes"` first — no half-measures.
